...
class MetalViewController: NSViewController {
	...
	let size = CGSize(width: 2 * 256 + 128, height: 2 * 256) // just needs to be divisible by 8
...
//			print("isHeadless: \(device.isHeadless)")
			...
			DispatchQueue.main.asyncAfter(deadline: DispatchTime(uptimeNanoseconds: 1_000_000)) {
				if let window = self.view.window {
					let origin = window.frame.origin
					window.setFrame(CGRect(origin: origin, size: self.frame.size), display: true, animate: true)
				}
			}
